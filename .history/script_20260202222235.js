document.addEventListener('DOMContentLoaded', function() {
    ...
    // DOM Elements - Partial Sell Calculator (NEW)
    const psTotalSharesInput = document.getElementById('psTotalShares');
    const psAvgPriceInput = document.getElementById('psAvgPrice');
    const psSellQuantityInput = document.getElementById('psSellQuantity');
    const psSellPriceInput = document.getElementById('psSellPrice');
    const psBrokerageInput = document.getElementById('psBrokerage');
    const psNetReceivedInput = document.getElementById('psNetReceived');
    const calculatePartialSellBtn = document.getElementById('calculatePartialSellBtn');
    const resetPartialSellBtn = document.getElementById('resetPartialSell');
    const partialSellResults = document.getElementById('partialSellResults');
    ...
    // PARTIAL SELL CALCULATOR (NEW FUNCTION)
    function calculatePartialSell() {
        // Get values
        const totalShares = parseFloat(psTotalSharesInput.value) || 0;
        const avgPrice = parseFloat(psAvgPriceInput.value) || 0;
        const sellQuantity = parseFloat(psSellQuantityInput.value) || 0;
        const sellPrice = parseFloat(psSellPriceInput.value) || 0;
        const brokerage = parseFloat(psBrokerageInput.value) || 0;
        const netReceived = parseFloat(psNetReceivedInput.value) || 0;
        
        // Validate
        if (totalShares <= 0 || avgPrice <= 0) {
            showNotification('Please enter valid total shares and average price', 'error');
            return;
        }
        
        if (sellQuantity <= 0 || sellPrice <= 0) {
            showNotification('Please enter valid sell quantity and sell price', 'error');
            return;
        }
        
        if (sellQuantity > totalShares) {
            showNotification('Sell quantity cannot exceed total shares owned', 'error');
            return;
        }
        
        // Calculate
        const costOfSold = sellQuantity * avgPrice;
        const grossSale = sellQuantity * sellPrice;
        
        // Calculate brokerage if not provided but net received is provided
        let actualBrokerage = brokerage;
        let actualNetReceived = netReceived;
        
        if (brokerage === 0 && netReceived > 0) {
            actualBrokerage = grossSale - netReceived;
            actualNetReceived = netReceived;
        } else if (brokerage > 0 && netReceived === 0) {
            actualNetReceived = grossSale - brokerage;
        } else {
            actualNetReceived = grossSale - brokerage;
        }
        
        const grossProfitLoss = grossSale - costOfSold;
        const netProfitLoss = actualNetReceived - costOfSold;
        const remainingShares = totalShares - sellQuantity;
        const roi = (netProfitLoss / costOfSold) * 100;
        
        // Update UI
        document.getElementById('psSoldQty').textContent = sellQuantity.toLocaleString();
        document.getElementById('psRemainingQty').textContent = remainingShares.toLocaleString();
        document.getElementById('psGrossAmount').textContent = 'Tk ' + grossSale.toFixed(2);
        document.getElementById('psProfitLossAmount').textContent = 'Tk ' + Math.abs(netProfitLoss).toFixed(2);
        
        // Update breakdown
        document.getElementById('psCostSold').textContent = 'Tk ' + costOfSold.toFixed(2);
        document.getElementById('psGrossSale').textContent = 'Tk ' + grossSale.toFixed(2);
        document.getElementById('psCharges').textContent = 'Tk ' + actualBrokerage.toFixed(2);
        document.getElementById('psNetAmount').textContent = 'Tk ' + actualNetReceived.toFixed(2);
        document.getElementById('psGrossPL').textContent = 'Tk ' + grossProfitLoss.toFixed(2);
        document.getElementById('psNetPL').textContent = 'Tk ' + netProfitLoss.toFixed(2);
        document.getElementById('psROI').textContent = roi.toFixed(2) + '%';
        
        // Determine status and apply styling
        const profitLossCard = document.getElementById('psProfitLossCard');
        const profitLossIcon = document.getElementById('psProfitLossIcon');
        const roiElement = document.getElementById('psROI');
        const statusElement = document.getElementById('psStatus');
        const messageElement = document.getElementById('psMessage');
        
        if (netProfitLoss > 0) {
            // Profit
            profitLossCard.className = 'stat-card profit';
            profitLossIcon.innerHTML = '<i class="fas fa-arrow-up"></i>';
            roiElement.className = 'performance-value positive';
            statusElement.textContent = 'Profit';
            statusElement.className = 'status-value profit';
            messageElement.textContent = 'Great! You made a profit on this partial sale.';
            messageElement.style.color = 'var(--success-color)';
            showNotification('Partial sale profit: Tk' + netProfitLoss.toFixed(2) + ' (' + roi.toFixed(2) + '% ROI)', 'success');
        } else if (netProfitLoss < 0) {
            // Loss
            profitLossCard.className = 'stat-card loss';
            profitLossIcon.innerHTML = '<i class="fas fa-arrow-down"></i>';
            roiElement.className = 'performance-value negative';
            statusElement.textContent = 'Loss';
            statusElement.className = 'status-value loss';
            messageElement.textContent = 'You incurred a loss on this partial sale. Consider holding remaining shares.';
            messageElement.style.color = 'var(--danger-color)';
            showNotification('Partial sale loss: Tk' + Math.abs(netProfitLoss).toFixed(2) + ' (' + roi.toFixed(2) + '% ROI)', 'warning');
        } else {
            // Break-even
            profitLossCard.className = 'stat-card highlight';
            profitLossIcon.innerHTML = '<i class="fas fa-equals"></i>';
            roiElement.className = 'performance-value';
            statusElement.textContent = 'Break-even';
            statusElement.className = 'status-value break-even';
            messageElement.textContent = 'You broke even on this partial sale. No profit or loss.';
            messageElement.style.color = 'var(--primary-color)';
            showNotification('Partial sale at break-even point', 'success');
        }
        
        document.getElementById('psResultsStatus').textContent = 'Calculated';
        
        // Show results
        partialSellResults.style.display = 'block';
    }
    ...
    function resetPartialSellCalculator() {
        psTotalSharesInput.value = '';
        psAvgPriceInput.value = '';
        psSellQuantityInput.value = '';
        psSellPriceInput.value = '';
        psBrokerageInput.value = '0';
        psNetReceivedInput.value = '';
        partialSellResults.style.display = 'none';
        showNotification('Partial sell calculator reset');
    }
    
    // LOAD EXAMPLES (Updated with Partial Sell Example)
    function loadExamples() {
        ...
        // Partial Sell Calculator Example (EBL Sale - YOUR EXAMPLE)
        psTotalSharesInput.value = '7250';
        psAvgPriceInput.value = '25.40';
        psSellQuantityInput.value = '2250';
        psSellPriceInput.value = '25.90';
        psBrokerageInput.value = '291.37';
        psNetReceivedInput.value = '57983.63';
    ...
    function initEventListeners() {
        ...
        // Partial Sell Calculator (NEW)
        calculatePartialSellBtn.addEventListener('click', calculatePartialSell);
        resetPartialSellBtn.addEventListener('click', resetPartialSellCalculator);
        
        // Auto-calculate brokerage for partial sell
        psSellPriceInput.addEventListener('blur', function() {
            const sellQuantity = parseFloat(psSellQuantityInput.value) || 0;
            const sellPrice = parseFloat(psSellPriceInput.value) || 0;
            
            if (sellQuantity > 0 && sellPrice > 0 && (!psBrokerageInput.value || psBrokerageInput.value === '0')) {
                const grossAmount = sellQuantity * sellPrice;
                const brokerage = grossAmount * 0.005; // 0.5%
                psBrokerageInput.value = brokerage.toFixed(2);
            }
        });
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                ...
                if (activeSection.id === 'average-calc' && 
                    ...
                    calculateGainLoss();
                } else if (activeSection.id === 'partialsell-calc' && 
                    (e.target === psTotalSharesInput || e.target === psAvgPriceInput || 
                     e.target === psSellQuantityInput || e.target === psSellPriceInput || 
                     e.target === psBrokerageInput || e.target === psNetReceivedInput)) {
                    calculatePartialSell();
                }
            }
        });
    ...
    function init() {
        ...
        initBonusTabs();
        initEventListeners();
        showNotification('Welcome to Zabers Stock Tools v5.0 with Partial Sell Calculator!', 'success');
        console.log('Stock Tools v5.0 initialized successfully!');
    }
